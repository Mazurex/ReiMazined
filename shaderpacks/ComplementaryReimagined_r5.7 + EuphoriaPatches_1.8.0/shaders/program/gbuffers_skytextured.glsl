...
void main() {
        ...
        if (isSun || isMoon) {
            ...
            #if MC_VERSION >= 11300
                // color.rgb *= color.rgb * smoothstep1(sqrt1(max0(VdotU))); // we actually don't want this effect for custom skyboxes
            #else
                discard;
        ...
        color.rgba *= max0(glColor.a + float(isSun || isMoon) * rainStrength); // Support fop skybox altering mods
    #endif
